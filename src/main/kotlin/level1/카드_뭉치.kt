...
문자열로 이루어진 배열 cards1, cards2와 원하는 단어 배열 goal이 매개변수로 주어질 때, cards1과 cards2에 적힌 단어들로 goal를 만들 있다면 "Yes"를, 만들 수 없다면 "No"를 return하는 level1.solution 함수를 완성해주세요.
...
private fun solution(cards1: Array<String>, cards2: Array<String>, goal: Array<String>): String {
    ...
    var l2 = cards2.toMutableList()

    goal.forEach { word ->
        if (l1.size>0&& word.equals(l1.get(0))){
            l1.removeAt(0)
        }else if(word.equals(l2.get(0))){
            l2.removeAt(0)
        }else{
            answer = "NO"
        }
